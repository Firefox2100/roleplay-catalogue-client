...
import type { CatalogueResource, CreateCharacterInput, Locale, SelectedCharacter } from "./types";
import "./ResourcePicker.css";

const emptyInput = (locale: Locale): CreateCharacterInput => ({ name: "", description: "", language: locale === "zh-CN" ? "zh-cn" : "en-uk", visibility: "private", tags: [] });
...
function ResourceCard({ resource, selected, busy, choose, t }: { resource: CatalogueResource; selected: boolean; busy: boolean; choose: (id: string) => Promise<void>; t: (key: MessageKey) => string }) {
  ...
  return <article className={`resource-card ${selected ? "selected" : ""}`}>
    ...
    <div className="resource-card-body"><div className="resource-card-info" tabIndex={description ? 0 : undefined} aria-describedby={description ? tooltipId : undefined}>
      <div className="resource-card-title"><h2>{resource.metadata.name}</h2><div className="resource-badges"><span>{t(resource.metadata.language === "zh-cn" ? "languageZhCN" : "languageEnUK")}</span><span>{t(resource.metadata.visibility as "private" | "authenticated" | "public")}</span></div></div>
...
export function ResourcePicker({ configured, selected, locale, onSelected, onOpenSettings, t }: { configured: boolean; selected: SelectedCharacter | null; locale: Locale; onSelected: (value: SelectedCharacter) => void; onOpenSettings: () => void; t: (key: MessageKey) => string }) {
  const [resources, setResources] = useState<CatalogueResource[]>([]), [loading, setLoading] = useState(false), [error, setError] = useState(""), [creating, setCreating] = useState(false), [form, setForm] = useState(() => emptyInput(locale)), [busyId, setBusyId] = useState("");
  ...
  const submit = async (event: React.FormEvent) => { event.preventDefault(); setBusyId("create"); setError(""); try { const result = await createCharacter(form); onSelected(result); setResources((items) => [result.resource, ...items]); setCreating(false); setForm(emptyInput(locale)); } catch (reason) { setError(String(reason)); } finally { setBusyId(""); } };
