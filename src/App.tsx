...
import { WORLD_CONFIGS, WORLD_SECTIONS, worldConfigLabel, worldSectionLabel, WorldBundlePage, type WorldPageKey } from "./WorldBundlePage";
import { threeWayMerge, type MergeConflicts } from "./threeWayMerge";
import "./Theme.css";

type Page = "resources" | "overview" | "playground" | "lorebook-overview" | "preset-overview" | "preset" | "world" | "foundation" | "scenes" | "dialogue" | "runtime" | "metadata" | "lorebook" | "linked-lorebooks" | "extensions" | "mvu" | "assets" | "settings" | WorldPageKey;
const desktopDefault = () => window.matchMedia("(min-width: 721px)").matches;
const fallback: BootstrapData = { version: "0.1.0", config: { locale: "en-GB", appearance: "system", llm: { provider: "openai", baseUrl: "https://api.openai.com/v1", apiKey: "", model: "gpt-4.1", contextWindow: 128000, maxOutputTokens: 4096, temperature: 0.7 }, catalogue: { baseUrl: "", apiKey: "" }, testing: { promptTemplate: "Write {{char}}'s next reply in a fictional roleplay with {{user}}. Stay consistent with the character and setting. Do not decide {{user}}'s actions, thoughts, or dialogue.\n\n{{system_prompt}}\n\n{{lore_before}}\n\nCharacter: {{description}}\nPersonality: {{personality}}\nScenario: {{scenario}}\n\n{{lore_after}}\n\nExample dialogue:\n{{mes_example}}" } } };
  ...
  return <div className={effectiveShellClass}>
    ...
    <aside className={`drawer ${leftOpen ? "drawer--open" : ""}`}>
      <nav className="nav-list">
        ...
        <button className={page === "playground" ? "active" : ""} onClick={() => go("playground")}><span>▷</span>{t("playgroundTitle")}</button>
        ...
        {selectedWorld && <><button className={page === "world-overview" ? "active" : ""} onClick={() => go("world-overview")}><span>◫</span>{t("wseOverviewTitle")}</button><button className={page === "world-details" ? "active" : ""} onClick={() => go("world-details")}><span>◎</span>{t("wseWorldAuthor")}</button>{WORLD_SECTIONS.map((section) => <button key={section} className={page === `world-section:${section}` ? "active" : ""} onClick={() => go(`world-section:${section}`)}><span>◇</span>{worldSectionLabel(t, section)}</button>)}{WORLD_CONFIGS.map((config) => <button key={config} className={page === `world-config:${config}` ? "active" : ""} onClick={() => go(`world-config:${config}`)}><span>⌁</span>{worldConfigLabel(t, config)}</button>)}<button className={page === "world-prompts" ? "active" : ""} onClick={() => go("world-prompts")}><span>≡</span>{t("wsePrompts")}</button><button className={page === "world-workflows" ? "active" : ""} onClick={() => go("world-workflows")}><span>▧</span>{t("wseWorkflows")}</button><button className={page === "world-media" ? "active" : ""} onClick={() => go("world-media")}><span>▧</span>{t("wseMedia")}</button></>}
      </nav>
    ...
    <main className="page">
      ...
      {page === "playground" && selectedCharacter && <CharacterPlaygroundPage selected={selectedCharacter} providerConfigured={Boolean(data.config.llm.baseUrl && data.config.llm.model && (data.config.llm.provider === "ollama" || data.config.llm.apiKey))} t={t} />}
...
function SettingsPage({ draft, status, llm, provider, setDraft, save, t }: { draft: AppConfig; status: "idle" | "saving" | "saved" | "error"; llm: <K extends keyof AppConfig["llm"]>(key: K, value: AppConfig["llm"][K]) => void; provider: (kind: ProviderKind) => void; setDraft: React.Dispatch<React.SetStateAction<AppConfig>>; save: () => Promise<void>; t: (key: MessageKey) => string }) {
  return <section className="settings-page"><div className="page-heading"><h1>{t("settingsTitle")}</h1><p>{t("settingsIntro")}</p></div>
    ...
    <section className="settings-card playground-settings-card"><h2>{t("playgroundPromptTemplate")}</h2><label><span className="sr-only">{t("playgroundPromptTemplate")}</span><textarea rows={14} value={draft.testing.promptTemplate} onChange={(event) => setDraft((current) => ({ ...current, testing: { promptTemplate: event.target.value } }))} /></label><p>{t("playgroundSettingsHint")}</p><small>{t("playgroundPlaceholders")}</small></section>
